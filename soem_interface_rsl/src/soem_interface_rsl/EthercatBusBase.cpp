...
namespace soem_interface_rsl
{
  ...
  struct EthercatBusBaseTemplateAdapter::EthercatSlaveBaseImpl
  {
    ...
    bool startup(std::atomic<bool> &abortFlag, const bool sizeCheck, int maxDiscoverRetries)
    {
      ...
      // We call map_group once with a small buffer - the return value tells us then how big the buffer actually has to be
      char temp_buffer[255];
      int ioMapSize = ecx_config_map_group(&ecatContext_, &temp_buffer, 0);
      MELO_DEBUG_STREAM("[soem_interface_rsl::" << name_ << "] Configured ioMap with size: " << ioMapSize);
      // We resize the buffer to what is actually needed
      ioMap_.resize(ioMapSize);
      // And call it again
      ioMapSize = ecx_config_map_group(&ecatContext_, ioMap_.data(), 0);
      // Sanity check for paranoid people
      if (static_cast<std::size_t>(ioMapSize) != ioMap_.size())
      {
        MELO_ERROR_STREAM("Critical error during dynamic ioMap size remapping");
        return false;
      }
